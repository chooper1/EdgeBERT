...
class BertForQuestionAnswering(BertPreTrainedModel):
    def __init__(self, config, params):
        super(BertForQuestionAnswering, self).__init__(config, params)
        self.num_labels = config.num_labels
        self.num_layers = config.num_hidden_layers
    ...
    Examples::
        ...
        if not self.training:
            original_entropy = (entropy(start_logits) + entropy(end_logits))/2
            highway_entropy = []
        ...
        if start_positions is not None and end_positions is not None:
            ...
            for highway_exit in outputs[-1]:
                highway_logits = highway_exit[0]
                start_logits, end_logits = highway_logits.split(1, dim=-1)
                if not self.training:
                    highway_logits_all.append(highway_logits)
                    highway_entropy.append(highway_exit[2])
                loss_fct = CrossEntropyLoss(ignore_index=ignored_index)
                start_loss = loss_fct(start_logits, start_positions) #fix this (all positions for one logit?)
                end_loss = loss_fct(end_logits, end_positions) #fix this
